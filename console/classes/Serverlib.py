...
class Server:
    def __init__(self, logging=True, ad_filtering=False, malware_block=False, porn_block=False, custome=False):
        """
    ...
    def __connect_request(self, url, client_socket):
        ...
        if ':' in url[1]:
            ...
            s.settimeout(0.3)
            port = int(port)
            if not self.is_allowed(webserver):
                return False
            if self.logging:
    ...
    def __start_database(self):
        ...
        if "malware" in self.blocks:
            try:
                malware_domains_file = open(files_dir + r"\Malware_domains.txt", 'r')
            except:
                try:
                    malware_domains_file = open(files_dir + r"/Malware_domains.txt", 'r')
                except:
        ...
        if "custome" in self.blocks:
            try:
                ...
                try:
                    ...
                    print("cant open custome domains file ")
                    return
    ...
    def is_custome_list(self, domain):

        if domain[0:4] == 'www.':
            if domain in self.database['custome'][domain[0].lower()]:
                if self.logging:
                    print("custome list request detected and blocked -" + domain)
                return True
            if domain[4::] in self.database['custome'][domain[0].lower()]:
                if self.logging:
                    print("custome list request detected and blocked -" + domain)
                return True
        else:
            if "www." + domain in self.database['custome'][domain[0].lower()]:
                if self.logging:
                    print("custome list request detected and blocked -" + domain)
                return True
    ...
    def is_allowed(self,webserver):
        for block in self.blocks:
            if block == 'ads':
                if self.is_ad_request(webserver):
                    return False
            if block == 'porn':
                if self.is_porn(webserver):
                    return False
            if block == 'malwares':
                if self.is_malware(webserver):
                    return False
            if block == 'custome':
                if self.is_custome_list(webserver):
                    return False
        return True
